...
- **答错的渠道现在会真的让位**：`tools/pelican-probe.py --kind bank --apply` 问六道答案各自独立验证过的题
  ...
  超时、鉴权失败、判不出一律什么都不动。整轮全对才解除。**按需跑，不挂定时器**（试过每 30 分钟一轮：24 小时
  112 轮 400 次提问，其中 195 次 403、101 次 502，四分之三的流量没带任何信息）；首轮就把官方车道隔离了——
  一道最小值可证为 21 的题它答了 29。`tools/ocx-probe.{service,timer}` 只作为想挂计划的人的可选模板。
